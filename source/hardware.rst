...
Многие процессоры AMD с архитектурами Zen2, Zen3 и новее поддерживают улучшенный механизм переключения тактовой частоты **amd-pstate**, позволяющий уменьшить энергопотребление процессора, снизить минимальную частоту в простое.

Для его включения надо прописать в строку параметров ядра:
...
Проверить работоспособность и узнать подробности можно командой:

.. code-block:: text

    cpupower frequency-info

Для этого должен быть установлен пакет **kernel-tools**.
Если ваш процессор не поддерживает данную технологию, то будет использован традиционный механизм **acpi-cpufreq**.
