...
В этом случае запустим утилиту **hdajackretask** из пакета **alsa-utils** (при отсутствии установим его -- ``sudo dnf install alsa-utils``), которая позволит нам легко и быстро переназначить выходы звуковой карты в соответствии с текущим подключением устройств вывода звука.
