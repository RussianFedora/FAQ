...
Как сделать многоканальный аналоговый аудиовыход?
===============================================================

В простейшем случае достаточно выбрать в настройках звука профиль **Аналоговый объёмный 5.1 выход** (2.1, 4.0, 4.1, 5.0, 7.1).

Если же доступен только стерео профиль, то, возможно, некоторые выходы звуковой карты отведены под микрофон и линейный вход. В этом случае надо запустить утилиту **hdajackretask** из пакета alsa-utils. Эта графическая программа позволит переназначить выходы звуковой карты в соответствии с вашим подключением динамиков. Интерфейс интуитивно понятен - выходы определяются по цвету (Green, Pink, Blue...) и расположению (Rear side, Front side). Там же можно назначить функции разъёмов на передней панели системного блока.

После внесения изменений нажмите **Install boot override** и перезагрузите систему. Теперь в настройках звуковой карты появятся требуемые профили объёмного вывода.
