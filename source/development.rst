...
Т.к. согласно гайдлайнам Fedora, все SRPM пакеты должны быть готовы к пересборке для любой поддерживаемой версии дистрибутива, мы не можем использовать условные операторы для директив *Source* и *Patch*, поэтому должны сначала заменим ``%autosetup`` на ``%setup -q``, а затем воспользуемся одним из описанных далее способов.
...
Изменим нумерацию патчей в директивах **Patch** так, чтобы от **1** до **9** были патчи, которые требуется применить при выполнении заданного условия, а с **10** -- все остальные:
