...
.. index:: rpm, rpmbuild, koji, fedpkg, build, bodhi
.. _koji-side-tags:

Как собрать несколько зависящих друг от друга пакетов?
==========================================================

Если раньше приходилось использовать механизм "build overrides", позволяющий вручную переопределять пакеты в сборочном репозитории, то сейчас рекомендуется для сборки двух и более пакетов применять side-теги, т.к. это более простой и безопасный способ, не создающий проблем с зависимостями.

Мейнтейнер может запросить создание любого количества side-тегов и производить сборку в них по своему усмотрению.
...
Дождёмся его создания:

.. code-block:: text

    koji wait-repo f35-build-side-XXXXX

Соберём первый пакет **foo**:
...
Собираем остальные пакеты тем же способом.
