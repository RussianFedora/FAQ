...
.. index:: git, commit, cherry-pick, backport
.. _git-backport-commits:

Как переместить набор коммитов из одной ветки Git-репозитория в другую?
==========================================================================

При необходимости переложить несколько коммитов из одной ветки **foo** в ветку **bar** (например, вы сделали некоторые правки в вашей ветке, которые не относятся к разрабатываемой вами функциональности), то мы можем запросить список из N коммитов из первой ветки, и сделать **cherry-pick** на второй ветке. Эти действия можно схлопнуть в одну конвейерную команду (пример приведен для 10 коммитов из ветки mybranch, его надо выполнять в другом каталоге, созданном через **git worktree add**. Пример приведен для Bash-совместимых shell, в которых также есть команды **tac** и **xargs**):

.. code-block:: text

    git log --format=%h -10 foo | tac | xargs git cherry-pick

Данная команда сначала запрашивает список из коммитов в формате "список хешей", затем разворачивает список с помощью команды **tac**, и затем каждую строчку списка передает команде **git cherry-pick**, которая впоследствие и применяет каждый коммит в нужную ветку.
