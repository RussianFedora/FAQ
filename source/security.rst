...
Создадим каталог для хранения ключей ``/etc/luks`` (может быть любым):
...
.. code-block:: text
    ...
    sudo dd if=/dev/urandom of=/etc/luks/foo-bar.key bs=1024 count=4

Установим корректные права доступа:

.. code-block:: text

    sudo chown root:root /etc/luks/foo-bar.key
    sudo chmod 0400 /etc/luks/foo-bar.key

Добавим ключ в свободный слот LUKS заголовка зашифрованного раздела:

.. code-block:: text

    sudo cryptsetup luksAddKey /dev/sda2 /etc/luks/foo-bar.key
...
.. code-block:: text
    ...
    sudo cryptsetup luksRemoveKey /dev/sda2 /etc/luks/foo-bar.key

Надёжно :ref:`уничтожим <destroy-file>` ключевой файл:

.. code-block:: text

    sudo shred -u -v /etc/luks/foo-bar.key
...
.. code-block:: text
    ...
    foo-bar UUID=XXXXXX /etc/luks/foo-bar.key luks

Здесь **foo-bar** - внутреннее имя, которое будет использоваться dev-mapper, **XXXXXX** - :ref:`UUID диска <get-uuid>`, **/etc/luks/foo-bar.key** - полный путь к :ref:`ключевому файлу <luks-addkey>`. При шифровании :ref:`SSD накопителя <get-uuid>` вместо параметра **luks** следует использовать **discard**.
...
.. code-block:: text
    ...
    foo-bar UUID=XXXXXX /etc/luks/foo-bar.key noauto

Здесь **foo-bar** - внутреннее имя, которое будет использоваться dev-mapper, **XXXXXX** - :ref:`UUID диска <get-uuid>`, **/etc/luks/foo-bar.key** - полный путь к :ref:`ключевому файлу <luks-addkey>`. Параметр **noauto** должен применяться только совместно с ключом.
...
Из-за несвободной лицензии TrueCrypt и все его форки (в т.ч. VeraCrypt) не могут быть добавлены в репозитории Fedora, однако в настоящее время утилита **cryptsetup** полностью их поддерживает.

Cryptsetup поддерживает монтирование TrueCrypt и VeraCrypt томов (файлы и устройства), умеет администрировать их (управлять ключами, паролями). Ограничение лишь одно - нельзя создавать новые зашифрованные данными механизмами контейнеры.
