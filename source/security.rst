...
.. code-block:: text
    ...
    gpg2 --output ~/.ssh/id_rsa.pub --export-ssh-key XXXXXXXXX

Здесь **XXXXXXXXX** - ID нашего ключа. Полученный публичный ключ нужно добавить на удалённый хост, например Github.
...
Настроим автоматический запуск GnuPG агента вместе с системой, добавив в конец файла ``~/.bashrc`` следующие строки:
...
Изменения будут применены после перезагрузки системы.
