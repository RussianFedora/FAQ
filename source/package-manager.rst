...
Как правильно установить определенную версию PHP, которая не поставляется в репозиториях?
==============================================================================================

Необходимо установить нужную версию PHP из репозитория Remi`s RPM.

Remi`s RPM - это сторонний репозиторий, созданный и поддерживаемый участником Fedora-сообщества по имени Remi Collect.
Основная цель Remi`s RPM Repo - предоставление версий стека PHP и некоторых других программ для пользователей Fedora и Enterprise Linux (RHEL, CentOS, Oracle, Scientific Linux,…)
...
.. code-block:: text
        ...
        sudo dnf install https://rpms.remirepo.net/fedora/remi-release-$(. /etc/os-release ; echo $VERSION_ID).rpm
...
Установим менеджер процессов PHP-FPM для PHP версии 7.3:
...
Обратите внимание, что префикс необходимо использовать и при взаимодействии с юнитами SystemD.
...
.. code-block:: text
    ...
    php73 /path/to/file.php

Обратите внимание, что префикс в данном случае является символьной ссылкой для вызова исполняемого файла PHP-интерпретатора нужной версии.
...
Обратите внимание, что здесь префикс используется как имя каталога инстанса PHP.

.. index:: php, use, several, versions
.. _php-use-several-versions:

Могу ли я использовать несколько версий PHP в системе одновременно?
=======================================================================

Да, это возможно, при :ref:`использовании репозитория Remi's RPM <php-remi-install>`.
