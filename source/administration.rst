...
Официально Docker в Fedora больше нет и не будет. На просторах интернета вы можете найти сторонние инструкции по установке Docker, но мы не можем порекомендовать следовать им, поскольку требуются изменения в системе способные нарушить безопасность или работу других приложений (например отключение cgroup).

Вместо этого рекомендуется установить и использовать Podman, т.к. он не требует прав суперпользователя для работы:
