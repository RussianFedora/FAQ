...
BTRFS использует двухуровневую структуру хранения данных: пространство поделено на *фрагменты*, которые содержат *блоки данных*. При длительной эксплуатации в файловой системе может возникать большое количество мало заполненных фрагментов. Это приводит к ситуации, когда свободное место вроде есть, а записать очередной файл на диск нельзя.

Операция балансировки выполняет перенос блоков между фрагментами, освободившиеся фрагменты удаляются. Официальная документация рекомендует выполнять балансировку регулярно.

Если во время балансировки пропадёт питание или перезагрузится компьютер, то раздел скорее всего потребует восстановления. Поэтому, сначала следует оценить выгоду от выполнения балансировки.

Состояние раздела можно оценить, выполнив команду:
...
BTRFS поддерживает прозрачное сжатие файлов на разделе. Можно выбрать один из трёх алгоритмов: **zstd**, **zlib**, **lzo**. Сжатие включается через указание параметров монтирования раздела.
...
В колонке параметров добавим опцию *compress=zstd*:

.. code-block:: text

    UUID=4f569164-1ab6-4c8e-8c17-48074da9b8b3 /   btrfs   compress=zstd,subvol=fedora   0 0
