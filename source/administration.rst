...
В открывшемся редакторе найдем строку, которая начинается со слова **linux**, переёдем в ее конец. Отступим пробел от последнего символа и добавим нужный параметр. 
Если параметров несколько, следует использовать пробел в качестве разделителя между ними.
