...
Как получить доступ к локальной сети через Wireguard?
======================================================

Чтобы через VPN Wireguard был доступен не только один клиентский компьютер, а вся его локальная сеть, надо сперва внести изменения на сервере.

В файл /etc/wireguard/wg0.conf добавим адрес подсети клиента через запятую, например 192.168.1.0/24:
...
После этого на клиенте с локальной сетью разрешим перенаправление трафика и включим маскарадинг в firewalld:
...
На клиенте, с которого требуется доступ, допишем подсеть в AllowedIPs через запятую. Если там уже 0.0.0.0/0, то ничего не требуется.
    
Проверим доступность компьютеров из локальной сети со стороны других клиентов wireguard (подставьте реально существующий адрес в вашей локальной сети):

.. code-block:: text

    ping 192.168.1.2
    
Если всё работает, то сделаем изменения постоянными:
