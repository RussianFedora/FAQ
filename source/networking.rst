...
С настройками по умолчанию, предустановленными пакетом **NetworkManager-config-connectivity-fedora**, NetworkManager проверяет наличие доступа к Интернету по таймеру каждые 5 минут посредством обращения к файлу **hotspot.txt** по протоколу HTTP и анализируя ответ сервера.
