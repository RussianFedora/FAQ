...
Для изменения настроек по умолчанию, откроем файл ``/etc/zram.conf`` в текстовом редакторе:

.. code-block:: text

    sudoedit /etc/zram.conf

Внесём изменения в переменную ``FACTOR``:

.. code-block:: text

    FACTOR=1

Здесь значение **1** означает использование всего объёма RAM, **0.25** - 25% и т.д.
