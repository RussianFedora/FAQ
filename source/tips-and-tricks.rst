...
Нужно ли выполнять балансировку раздела BTRFS?
=================================================

:ref:`BTRFS <fs-btrfs>` использует двухуровневую структуру хранения данных: пространство поделено на *фрагменты*, которые содержат *блоки данных*. При определенных условиях эксплуатации в ФС может возникать большое количество мало заполненных фрагментов. Это приводит к ситуации, когда свободное место вроде есть, а записать очередной файл на диск нельзя.

Операция балансировки выполняет перенос блоков между фрагментами, освободившиеся фрагменты удаляются. Официальная документация `SysadminGuide <https://btrfs.wiki.kernel.org/index.php/SysadminGuide#Balancing>`_ рекомендует выполнять балансировку регулярно. Однако разработчики Fedora `против <https://pagure.io/fedora-btrfs/project/issue/16>`_ такого подхода.

Если у вас на разделе мало свободного места (меньше 20%), идёт интенсивная запись данных, например от СУБД, и вы столкнулись с ошибкой записи, то скорее всего балансировка вам поможет.

Также можно оценить выгоду от выполнения балансировки командой:

.. code-block:: text

    sudo btrfs fi usage [mountpoint]

Если **Device allocated** значительно превышает **Used**, то балансировка будет полезной.

.. index:: btrfs, balance, execution
.. _btrfs-make-balance:

Как выполнить балансировку раздела BTRFS?
============================================

Следующая команда выполняет балансировку для всех фрагментов, заполненных менее чем наполовину:

.. code-block:: text

    sudo btrfs fi balance start -dusage=50 -musage=50 [mountpoint]

**-dusage** - относится к балансировке данных, **-musage** - к метаданным. Чем меньше значение *usage*, тем быстрее выполняется операция.

Если на диске мало свободного места, то начинать следует с небольших значений, например с 5, постепенно увеличивая это число. Также можно балансировать отдельно данные и метаданные.

Подробнее о балансировке можно прочитать в официальной документации:

* `SysadminGuide <https://btrfs.wiki.kernel.org/index.php/SysadminGuide#Balancing>`_
* `Problem_FAQ <https://btrfs.wiki.kernel.org/index.php/Problem_FAQ#I_get_.22No_space_left_on_device.22_errors.2C_but_df_says_I.27ve_got_lots_of_space>`_

.. index:: btrfs, compression
.. _btrfs-compression:

Как включить сжатие раздела BTRFS?
=====================================

Начиная с *Fedora 34* для новых установок уже `включено <https://fedoraproject.org/wiki/Changes/BtrfsTransparentCompression>`_ сжатие для разделов :ref:`BTRFS <fs-btrfs>` алгоритмом **zstd** с уровнем сжатия 1. Для тех, кто обновляется с предыдущих релизов, сжатие можно включить самостоятельно.

:ref:`BTRFS <fs-btrfs>` поддерживает прозрачное сжатие файлов на разделе. Можно выбрать один из трёх алгоритмов: **zstd**, **zlib**, **lzo**. Сжатие включается через указание параметров монтирования раздела.

Включим сжатие для корневого раздела. Для этого отредактируем fstab:

.. code-block:: text

    sudoedit /etc/fstab

В колонке параметров добавим опцию *compress=zstd:1*:

.. code-block:: text

    UUID=4f569164-1ab6-4c8e-8c17-48074da9b8b3 /   btrfs   compress=zstd:1,subvol=fedora   0 0

Сохраним и перезагрузимся. После этого записываемые на раздел файлы будут сжиматься алгоритмом zstd.

Выгоду от использования сжатия можно оценить утилитой *compsize*:

.. code-block:: text

    sudo compsize -x /

Подробнее о сжатии можно узнать в официальной документации:

* `Compression <https://btrfs.wiki.kernel.org/index.php/Compression>`_
