...
Увеличивать размер пула выше стандартного значения **0.5** категорически не рекомендуется, т.к. это может приводить к зависаниям системы.
...
Здесь значение **0.25** означает выделение под пул 25% от оперативной памяти, **0.5** - 50% (выбор по умолчанию), **0.75** - 75% и **1** - 100% соответственно (не рекомендуется).
