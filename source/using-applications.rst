...
Произведём поиск и замену во всех файлах, удовлетворяющих маске имени ``*.txt``, начиная с текущего каталога:

.. code-block:: text

    find -type f -name "*.txt" -exec sed -e "s/foo/bar/g" -i "{}" \;

Внимание! В случае использования системы контроля версий Git не следует запускать процесс без указания маски имени, либо стартового каталога, т.к. это может привести к внесению правок в индексы, находящиеся внутри служебного каталога ``.git``, что приведёт к повреждению локальной копии репозитория.
