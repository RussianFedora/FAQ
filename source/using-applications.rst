...
.. index:: kde, chromium, chrome, default
.. _kde-chromium-default:

Как установить браузер chromium для использования по умолчанию?
==========================================================

Даже если в параметрах системы вы указали браузер по умолчанию chromium, сам он всё равно будет предалагать сделать это при каждом запуске. Чтобы избавиться от данного сообщения, следует сделать следующее:
Открыть Параметры системы - Приложения - Привязки файлов, в строке поиска ввести **html** и установить нужный браузер для типов **xhtml+xml** и **html**.

Chromium больше не будет считать, что он не является браузером по умолчанию.
